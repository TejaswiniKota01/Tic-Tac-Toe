...
frame1 = Frame(root)
frame1.pack()
titleLabel = Label(frame1 , text="Tic Tac Toe" , font=("Arial" , 26)  , width=30 )
...
def restartGame():
    ...
    titleLabel = Label(frame1 , text="Tic Tac Toe" , font=("Arial" , 30) , bg="peachpuff1" , width=40 )
    titleLabel.grid(row=0 , column=0)
...
def play(event):
    ...
    if button["text"] == " ":
        if turn == "x":
            board[clicked] = turn
            if checkForWin(turn):
                winningLabel = Label(frame1 , text=f"{turn} wins the game", bg="peachpuff1", font=("Arial" , 26),width=16 )
        ...
        if checkForDraw():
            drawLabel = Label(frame1 , text="Game Draw" , bg="peachpuff1", font=("Arial" , 26), width = 16)
...
singlePlayerButton.grid(row=0, column=0, padx=(0, 50), pady=3)
...
multiPlayerButton.grid(row=0, column=1, padx=(50, 0), pady=3)
...
button1 = Button(
    frame2, text=" ", width=4, height=2, font=("Arial", 30), bg="white", relief=RAISED, borderwidth=5
)
